...
export function InventoryList({ onViewProduct, searchQuery }: InventoryListProps) {
  ...
  const lowStockAlerts = products.filter(
    (p) => (Number(p.stock) || 0) <= (Number(p.maxStock) || 0) * 0.2
  ).length;

  const movementTotalsByProduct = useMemo(() => {
    return movements.reduce<Record<string, { entries: number; exits: number }>>((acc, movement) => {
      const qty = Number(movement.quantity) || 0;

      if (movement.type === 'entry' || movement.type === 'initial') {
        if (!acc[movement.productId]) acc[movement.productId] = { entries: 0, exits: 0 };
        acc[movement.productId].entries += qty;
      }

      if (movement.type === 'exit') {
        if (!acc[movement.productId]) acc[movement.productId] = { entries: 0, exits: 0 };
        acc[movement.productId].exits += qty;
      }

      if (movement.type === 'product_transfer') {
        if (!acc[movement.productId]) acc[movement.productId] = { entries: 0, exits: 0 };
        acc[movement.productId].exits += qty;

        if (movement.targetProductId) {
          if (!acc[movement.targetProductId]) acc[movement.targetProductId] = { entries: 0, exits: 0 };
          acc[movement.targetProductId].entries += qty;
        }
      }
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 text-center space-y-8 animate-in fade-in duration-500">
        <div className="w-24 h-24 bg-surface-container rounded-full flex items-center justify-center mb-4 relative">
          ...
          <div className="absolute inset-0 border-2 border-dashed border-outline-variant/30 rounded-full animate-[spin_15s_linear_infinite]" />
        </div>

        <div className="max-w-md space-y-3">
          <h2 className="text-3xl md:text-4xl font-black font-headline tracking-tighter text-on-surface uppercase">
            Inventário Zerado.
          </h2>
          <p className="text-on-surface-variant text-sm md:text-base leading-relaxed">
            Não há ativos registrados no sistema. O monitoramento de precisão começará assim que você
            adicionar itens ao seu catálogo.
          </p>
        </div>
        ...
        <ProductModal isOpen={isModalOpen} onClose={handleCloseModal} initialData={editingProduct} />
      </div>
    );
  }
    ...
    <div className="p-4 md:p-10 space-y-6 md:space-y-8 max-w-7xl mx-auto w-full">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="max-w-2xl">
          ...
          <p className="text-on-surface-variant mt-3 text-sm md:text-lg max-w-lg leading-relaxed">
            Uma visão curada da sua distribuição global de estoque. Monitore a avaliação e métricas de
            saúde em todos os nós.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-surface-container-low p-5 md:p-6 rounded-xl flex items-center justify-between group cursor-pointer hover:bg-surface-container transition-colors shadow-sm border border-white/5">
          <div>
            <p className="text-[10px] md:text-xs text-on-surface-variant uppercase tracking-widest mb-2 font-bold">
              Entradas Recentes
            </p>
        ...
        <div className="glass-panel p-4 rounded-xl flex flex-wrap items-center gap-4 md:gap-6 border border-white/5 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 hide-scrollbar">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1 rounded-full bg-secondary/10 text-secondary text-[10px] font-black uppercase whitespace-nowrap">
                Em Estoque
              </button>
              <button className="px-3 py-1 rounded-full hover:bg-white/5 text-on-surface-variant text-[10px] font-black uppercase whitespace-nowrap">
                Baixo Estoque
              </button>
              <button className="px-3 py-1 rounded-full hover:bg-white/5 text-on-surface-variant text-[10px] font-black uppercase whitespace-nowrap">
                Esgotado
      ...
      <div className="bg-surface-container-low rounded-2xl overflow-hidden shadow-2xl border border-white/5">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[1050px] md:min-w-0">
            <thead>
              <tr className="bg-surface-container text-on-surface-variant text-[10px] uppercase tracking-[0.2em] font-bold">
                ...
                <th className="px-6 py-5">Qtd. Entrada</th>
                <th className="px-6 py-5">Qtd. Saída</th>
            ...
            <tbody className="text-sm">
              {loading ? (
                ...
                <tr>
                  <td colSpan={10} className="py-20 text-center text-on-surface-variant font-bold uppercase tracking-widest">
                ...
                filteredProducts.map((item, idx) => {
                  ...
                  const totalStockVal = sorted.reduce(
                  ...
                  const cumVal = sorted
                    .slice(0, pos + 1)
                    .reduce((acc, p) => acc + (Number(p.price) || 0) * (Number(p.stock) || 0), 0);
                  const cumPercent = cumVal / (totalStockVal || 1);
                  const rank = cumPercent <= 0.8 ? 'A' : cumPercent <= 0.95 ? 'B' : 'C';
                  const movementTotals = movementTotalsByProduct[item.id] || { entries: 0, exits: 0 };
                  const isLowStock = (Number(item.stock) || 0) <= (Number(item.maxStock) || 0) * 0.2;
                  ...
                  return (
                    ...
                    <td className="px-6 md:px-8 py-6 font-mono text-[10px] md:text-xs text-on-surface-variant relative">
                      ...
                      <td className="px-6 py-6 font-bold text-secondary">{movementTotals.entries}</td>
                      <td className="px-6 py-6 font-bold text-tertiary">{movementTotals.exits}</td>
                      <td className="px-6 py-6">
                        <div className="flex flex-col gap-1.5">
                          <span className={cn('font-bold', isLowStock ? 'text-tertiary' : '')}>{Number(item.stock) || 0}</span>
                          <div className="w-16 h-1 bg-surface-bright rounded-full overflow-hidden">
                            <div
                              className={cn('h-full', isLowStock ? 'bg-tertiary' : 'bg-secondary')}
                              style={{
                                width: `${Math.min(100, ((Number(item.stock) || 0) / (Number(item.maxStock) || 1)) * 100)}%`,
                              }}
                            />
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-6 font-bold text-on-surface-variant">
                      ...
                      <td className="px-6 py-6 font-black text-secondary">
                        ...
                        {(
                          (Number(item.price) || 0) *
                          (Number(item.stock) || 0) *
                          (1 + ((Number(item.icms) || 0) + (Number(item.ipi) || 0)) / 100)
                      ...
                      <td className="px-6 py-6 hidden sm:table-cell">
                        <span
                          className={cn(
                            ...
                            !isLowStock ? 'bg-secondary/10 text-secondary' : 'bg-tertiary/10 text-tertiary'
                          )}
                        >
                          {!isLowStock ? 'Estável' : 'Baixo Estoque'}
                        </span>
                      </td>
